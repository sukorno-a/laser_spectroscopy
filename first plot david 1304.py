...
smooth = gaussian_filter1d(first_peak[3], 400)
smooth_d1 = np.gradient(smooth)
smooth_d2 = np.gradient(smooth_d1)
fig, ax = plt.subplots(figsize=(10,6))
plt.plot(first_peak[0],smooth)
plt.plot(first_peak[0],smooth_d2/max(np.max(smooth_d2),-np.min(smooth_d2)))
ax.axis(True)
sns.set(style="ticks")
ax.grid(True, which='both')
ax.margins(x=0)
...
smooth = gaussian_filter1d(new_splice[3], 400)
smooth_d1 = np.gradient(smooth)
smooth_d2 = np.gradient(smooth_d1)

peaks, _ = find_peaks(-smooth_d2, distance=1100)

rcParams["font.family"] = 'Times New Roman'
rcParams['mathtext.fontset'] = 'cm'
fig, ax = plt.subplots(figsize=(10,6))
plt.plot(new_splice[0],smooth_d2/np.max(smooth_d2),color="blue",label="Second Derivative")
plt.scatter(new_splice[0][19635+peaks],smooth_d2[peaks]/np.max(smooth_d2),color="blue",label="Minimum Points")
